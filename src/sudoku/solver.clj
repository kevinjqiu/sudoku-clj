(ns sudoku.solver
  :use [sudoku.const :only [in? *squares* *peers* *digits* *units*]])
...
(defn- try-assign-digit-in-unit [grid-ref current-unit digit]
  ...
  (let [places-for-digit (filter #(in? (get @grid-ref %) digit) current-unit)]
    (cond (empty? places-for-digit) false ;; Contradiction: no place for digit
  ...
  (let [digits (get @grid-ref square)]
    (if (not (in? (digits digit))) true
      (let [new-values (eliminated digits digit)]
        (cond (empty? new-values) false ;; Contradiction: last digit removed
              (= 1 (count new-values))
                ;; if a square is reduced only to a single value, 
                ;; eliminate that value from its peers.
                (if (not (every? true? (map #(eliminate grid-ref % new-values) *peers*))) false
...
(defn parse-grid [grid]
  ...
  (let [grid (ref (create-init-grid))
        values (grid-values grid)]
    (if (every? true? (map #(assign grid-ref (first %) (last %))))
      @grid-ref 
      false)))
